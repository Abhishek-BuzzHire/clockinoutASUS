      ...
      <div className="mt-4 text-md">
        {menuItems.map((i) => (
          <div className="flex flex-col gap-2" key={i.title}>
            <span className="hidden lg:block group-[.is-mobile]:block text-sm text-gray-400 font-light my-4">{i.title}</span>
            {i.items.map((item) => {
              ...
              return (
                <Link
                  href={href}
                  key={item.label}
                  className={`flex items-center justify-center lg:justify-start group-[.is-mobile]:justify-start gap-4 text-gray-600 font-semibold py-2 md:px-2 rounded-md ${isActive ? "text-white bg-indigo-600 " : "hover:text-gray-800 hover:bg-sky-100"}`}
                >
                  <Image src={item.icon} alt="" width={20} height={20} className={isActive ? "invert brightness-0" : ""} />
                  <span className="hidden lg:block group-[.is-mobile]:block">{item.label}</span>
                </Link>
              );
            })}
          </div>
