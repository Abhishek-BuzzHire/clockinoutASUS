...
const PunchCard: React.FC<{
    ...
    return (
        <>
            {/* MOBILE VIEW EXACT DESIGN (Hidden on Desktop) */}
            <div className="block lg:hidden bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden mb-4">
                    ...
                    <div className="flex justify-between items-start gap-2">
                        <div className="flex gap-3.5 items-center">
                            <div className="relative shrink-0">
                                <img
                                    ...
                                    className="w-16 h-16 sm:w-18 sm:h-18 rounded-full object-cover border-2 border-white shadow-xs bg-slate-50"
                                    onError={(e) => {
                                        e.currentTarget.src = "https://ui-avatars.com/api/?name=" + encodeURIComponent(profileName || "Employee") + "&background=0D8ABC&color=fff";
                                    }}
                                />
                                <div className={`absolute bottom-0 right-0 w-3.5 h-3.5 rounded-full border-2 border-white ${isPunchedIn ? "bg-green-500" : "bg-slate-400"}`}></div>
                            </div>

                            <div>
                                <h2 className="font-bold text-lg sm:text-xl text-slate-800">{profileName ?? "Shivam Yadav"}</h2>
                                <span className={`text-xs px-2.5 py-0.5 rounded-full inline-flex items-center gap-1 font-semibold mt-1.5 ${isPunchedIn ? "bg-green-100 text-green-600" : "bg-slate-100 text-slate-500"}`}>
                                ...
                                <p className="text-slate-400 text-xs mt-1 font-medium">
                                    {isPunchedIn ? `Punched in at ${punchTime || "09:29 AM"}` : `Last punched at ${punchTime || "--:--"}`}
                                </p>
                            </div>
                        </div>
                        ...
                        {onRefreshLocation && (
                            <button
                                ...
                                className="flex flex-col items-center justify-center p-2 rounded-2xl bg-slate-50/90 hover:bg-slate-100 border border-slate-200/60 transition text-slate-700 active:scale-95 cursor-pointer shadow-2xs w-20 shrink-0 text-center"
                            >
                                <div className="w-7 h-7 rounded-full bg-white shadow-2xs flex items-center justify-center mb-1 text-slate-700">
                                    <MapPin size={15} />
                                </div>
                                <span className="text-[10px] font-bold leading-tight">
                                    {isProcessingLocation ? "Locating..." : "Refresh Location"}
                                </span>
                            </button>
                        )}
                    </div>
                ...
                <div className="border-t border-slate-100 p-4 sm:p-5 bg-slate-50/30">
                    <div className="flex items-center justify-between gap-3">
                        <div>
                            <p className="text-slate-400 uppercase text-[10px] font-bold tracking-wider">Time Elapsed</p>
                            <div className="text-3xl sm:text-4xl font-extrabold text-slate-800 tracking-tight mt-0.5 font-mono">
                                {!isPunchedIn && hasPunchedOut ? formatTime(workedSeconds ?? 0) : formatTime(elapsedTime)}
                            </div>
                            <p className="text-[10px] text-slate-400 font-medium mt-0.5">Hrs : Mins : Secs</p>
                        </div>

                        <div className="shrink-0">
                            {!isPunchedIn && hasPunchedOut ? (
                                <button disabled className="bg-green-500 text-white px-5 py-3.5 rounded-2xl font-bold text-xs shadow-sm opacity-80 cursor-not-allowed">
                                    Punched Out
                                </button>
                            ) : (
                                <button
                                    onClick={handlePunchAction}
                                    className={`px-6 sm:px-8 py-3.5 rounded-2xl font-bold text-sm text-white shadow-md flex items-center gap-2 active:scale-95 transition-all ${
                                        isPunchedIn ? "bg-[#2F5BFF] hover:bg-blue-700 shadow-blue-200" : "bg-[#22C55E] hover:bg-green-600 shadow-green-200"
                                    }`}
                                >
                                    <span>{isPunchedIn ? "Clock Out" : "Clock In"}</span>
                                    {isPunchedIn ? <LogOut size={18} /> : <LogIn size={18} />}
                                </button>
                            )}
                        </div>
                    </div>
            ...
            {/* DESKTOP VIEW EXACT DESIGN (Hidden on Mobile) */}
            <div className="hidden lg:block relative w-full max-w-sm mx-auto bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 mt-0 mb-6">
                        ...
                        <div className="text-lg w-full px-4 pb-4 pt-1">
                            ...
                            <div className="flex flex-col-reverse lg:flex-row gap-4 pt-0">
                                ...
                                <div className="w-full lg:w-[20%] mt-0 relative space-y-4">
                                    ...
                                    <div className="bg-white rounded-3xl p-4 sm:p-5 shadow-sm border border-slate-100 mb-0 lg:mb-4">
                                        <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-3">Quick Actions</h3>
                                        <div className="grid grid-cols-3 gap-3">
                                            <button onClick={() => setOpenApplyLeaves(true)} className="flex flex-col items-center justify-center gap-2 p-3.5 rounded-2xl bg-blue-50/90 text-blue-600 hover:bg-blue-100 border border-blue-100/60 shadow-2xs active:scale-95 transition cursor-pointer">
                                                <CalendarPlus className="w-5 h-5" />
                                                <span className="text-xs font-bold">Leave</span>
                                            </button>
                                            <button onClick={() => setOpenApplyWfh(true)} className="flex flex-col items-center justify-center gap-2 p-3.5 rounded-2xl bg-purple-50/90 text-purple-600 hover:bg-purple-100 border border-purple-100/60 shadow-2xs active:scale-95 transition cursor-pointer">
                                                <Laptop className="w-5 h-5" />
                                                <span className="text-xs font-bold">WFH</span>
                                            </button>
                                            <button onClick={() => setOpenRegulize(true)} className="flex flex-col items-center justify-center gap-2 p-3.5 rounded-2xl bg-orange-50/90 text-orange-600 hover:bg-orange-100 border border-orange-100/60 shadow-2xs active:scale-95 transition cursor-pointer">
                                                <ClockArrowUp className="w-5 h-5" />
                                                <span className="text-xs font-bold">Regularize</span>
                                            </button>
                                        </div>
                                    </div>
